<template>
  <div :class="[`v-step-warp-${direction}`, styleType]">
    ...
    <ul class="v-step-list">
      <li v-for="(label, index) in steps" :key="index" class="v-step-item" :style="horizontalItemWidth"
        @click="selectItem(index)">
        <label class="v-step-item-label" :style="itemLabelStyle(index)">
          {{ label.title }}
        </label>
    ...
    computed: {
      ...
      progressStyle() {
        ...
        if (this.direction === 'vertical') {
          oStyle.height = this.stepItemWidth * this.active + '%';
        } else {
          oStyle.width = this.stepItemWidth * this.active + '%';
        }
        return oStyle;
      }
    },
